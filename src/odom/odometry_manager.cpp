...
namespace cocolic
{
  ...
  OdometryManager::OdometryManager(const YAML::Node &node, ros::NodeHandle &nh)
      // lrq
      : m_minumum_rgb_pts_size(0.05),  // 直接在初始化列表中初始化
      m_recent_visited_voxel_activated_time(0.0),
      m_pub_pt_minimum_views(5),
      m_map_output_dir("/home/lrq/catkin_ws/src/Coco-LIC"),
  ...
  void OdometryManager::RunBag(pcl::PointCloud<pcl::PointXYZI> &map_cloud)
  {
    size_t sum_points = 0;  //  记录点数量
    while (ros::ok())
    {
  ...
  void OdometryManager::SolveLICO(pcl::PointCloud<pcl::PointXYZI> &map_cloud)
  {
  ...
  void OdometryManager::ProcessLICData(pcl::PointCloud<pcl::PointXYZI> &map_cloud)
  {
    ...
    if (process_image)
    {
      ...
      if (odom_viewer_.pub_track_img_.getNumSubscribers() != 0 || odom_viewer_.pub_sub_visual_map_.getNumSubscribers() != 0)
      {
        ...
        for (const auto& point : visual_sub_map_debug)
        {
        ...
        //pcl::io::savePCDFileASCII(file_path, pcl_cloud);
        //std::cout << "Saved " << pcl_cloud.size() << " data points to visual_sub_map_debug.pcd." << std::endl;
    ...
    if (cloud_distort->size() != 0)
    {
      ...
      //The dedistortion point cloud is stored in cloud_undistort
      //size_t num_points = cloud_undistort->size();
      //std::cout << "\nNumber of points in the undistorted cloud: " << num_points << "\n";
      //std::cout << "\nThe type of the cloud is: " << typeid(cloud_undistort->points[0]).name() << "\n";
      //char ch = odom_manager.cv_keyboard_callback();

      // 创建一个存储 RGB_pt_ptr 的容器
      //std::vector<RGB_pt_ptr> undistorted_points;
      ...
      //camera_handler_->cv_keyboard_callback();
    }
    //std::cout << "\nThe total number of points in the cloud to undistortion points:" << sum_points << "\n";
  ...
  // lrq
  cv::Mat OdometryManager::generate_control_panel_img()
  {
    cout << "generate_control_panel_img" << endl;
    int     line_y = 40;
    int     padding_x = 10;
  ...
  // lrq
  char OdometryManager::cv_keyboard_callback()
  {
    cout << "cv_keyboard_callback" << endl;
    ...
    m_map_rgb_pts.save_and_display_pointcloud( std::string("/home/dell/test"), std::string("/rgb_pt"), 3 );
